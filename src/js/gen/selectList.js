...
* @Last Modified time: 2016-11-29 16:14:55
* @Function:
* @Describtion:
*/
...
fs.readdir(resultPath,function(err, files){//读取目录，用于判断即将创建的目录是否存在
    ...
    if(toMkdir){
        fs.mkdir(resultPath + dataPathName,function(err){
            if (err) {
                console.log("[GenData]  创建data目录失败！");
                return console.error(err);
            };
            console.log("[GenData]  创建data目录成功！");
            putInJson(resultPath + dataPathName);

        });
    }else {
        console.log('[GenData]  ' +resultPath + dataPathName + "目录已存在！");
        putInJson(resultPath + dataPathName);
    };
...
function putInJson(path) {
    ...
    writerStream.on('finish', function() {// 处理流事件 --> data, end, and error
        console.log("[GenData]  数据写入完成！");
    }).on('error', function(err){
        console.log(err.stack);
        console.log('[GenData]  数据写入失败！');
    });
};
